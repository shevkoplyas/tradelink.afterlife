...
namespace TradeLibFast
{
	...
	private:
		...
		struct TLOrderInfo
		{
			...
			TLOrderInfo(): id(0), ack(false) {}
			TLOrderInfo(int64 tlid, const CString& acct, bool acked): id(tlid), account(acct), ack(acked) {}
		};
		vector<TLOrderInfo> tlorders;

		typedef multimap<OrderId, TLTrade> deferred_fill_map_type;
		deferred_fill_map_type deferredFills;

		OrderId TL2IBID(int64 tlid);
		int findTLOrder(int64 tlid);
		...
		// dimon: also look at the diff between these 2 files: 
