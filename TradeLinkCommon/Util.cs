...
namespace TradeLink.Common
{
    ...
    public class Util
    {
        ...
        public static bool openurl(string url) { return openurl(url, null); }
        /// <summary>
        /// opens a url
        /// </summary>
        /// <param name="url"></param>
        /// <param name="d"></param>
        public static bool openurl(string url, DebugDelegate d)
        {
            try
            {
            ...
            {
                if (d != null)
                    d("unexpected error opening url: " + url + " err: " + ex.Message + ex.StackTrace);
            }
            return false;
        }
        ...
        public static string cjoin(params string[] items)
        {
            return string.Join(",", items);
        }

        public static string cjoin(params object[] items)
        {
            return string.Join(",", items);
        }
        ...
        /// <param name="d"></param>
        /// <returns></returns>
        public static string geturl(string url, DebugDelegate d) { return geturl(url, false, d); }
        /// <summary>
        /// gets a url
        /// </summary>
        /// <param name="url"></param>
        ...
        /// <param name="d"></param>
        /// <returns></returns>
        public static string geturl(string url, bool nocache, DebugDelegate d)
        {
            _deb = d;
            System.Net.HttpWebRequest wc = null;
            string r = string.Empty;
            try
            {
                wc = (System.Net.HttpWebRequest)System.Net.HttpWebRequest.Create(url);
                if (nocache)
            ...
            {
                debug("error getting: " + url + " " + ex.Message + ex.StackTrace);
                try
                {
                    if (wc != null)
                        wc.GetResponse().Close();
        ...
        private static string GetResponse(System.Net.HttpWebRequest req)
        {
        ...
        public static string TLVersion()
        {
            ...
            var fixs = fix < 10 ? "0" + fix : fix.ToString();
            string nicebuild = maj+"."+min+"."+fixs;
            return nicebuild;
        }
        /// <summary>
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public static long ToTLDateTime(DateTime dt) { return ((long)TradeLink.Common.Util.ToTLDate(dt) * 1000000) + TradeLink.Common.Util.ToTLTime(dt); }
        /// <summary>
        ...
        /// <returns></returns>
        [Obsolete("Use ResponseLoader.GetResponseList instead")]
        ...
        {
            return ResponseLoader.GetResponseList(dllfilepath, d);
        }
        /// <summary>
        ...
        {
            return ResponseLoader.GetResponseList(responseassembly, deb);
        }
        ...
        public static bool SaveCsv(List<string> header, List<List<string>> data, string fn, DebugDelegate d) { return SaveCsv(header, data.ToArray(), fn, 0, d); }
        public static bool SaveCsv(List<string> header, List<List<string>> data, string fn, int maxrec, DebugDelegate d) { return SaveCsv(header, data.ToArray(), fn, maxrec, d); }
        public static bool SaveCsv(List<string> header, List<string>[] data, string fn, DebugDelegate d) { return SaveCsv(header, data, fn, 0, d); }
        public static bool SaveCsv(List<string> header, List<string>[] data, string fn, int maxrec, DebugDelegate d)
        {
            _deb = d;
            try
            {
            ...
            {
                debug("error writting file: " + fn + " err: " + ex.Message + ex.StackTrace);
                return false;
            }
        ...
        public static List<List<string>> ParseCsvData(string data, DebugDelegate deb) { return ParseCsvData(data, 1, deb); }
        public static List<List<string>> ParseCsvData(string data, int startline, DebugDelegate deb)
        {
        ...
        public static List<List<string>> ParseCsv(string fn, DebugDelegate deb)
        {
            string data = getfile(fn, deb);
            return ParseCsvData(data, deb);

        }
        public static string SymbolsToSymArray(string symlistname)
        {
            return "BasketImpl.FromString(" + symlistname + ").ToSymArray()";
        }

        public static List<string> ParseCsvHeaderFile(string fn)
        {
            string data = getfile(fn, null);
            return ParseCsvHeaderData(data);
        }

        public static List<string> ParseCsvHeaderData(string data) { return ParseCsvHeaderData(data, null); }
        public static List<string> ParseCsvHeaderData(string data, DebugDelegate deb)
        {
            _deb = deb;
            string[] lines = data.Split(Environment.NewLine.ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
            if (lines.Length == 0)
            {
                debug(data.Length + " could not be parsed for csv header, contains no newline.");
        ...
        public static string[] getr(string line) { return getr(line, ","); }
        public static string[] getr(string line, string delim)
        {
            return getrec(line, delim);
        }
        ...
        {
            int eos = line.IndexOf('\"', start + 1);
            if (eos < 0)
                return line.Length - 1;
            return eos;
        }
        static int geteod(int start, string line, string delim)
        {
            int eod = line.IndexOf(delim, start);
            if (eod == -1)
                eod = line.Length;
            return eod;
        }
        ...
        {
            return line.Substring(start, eod - start);
        }
        static string getstring(int start, int eos, string line)
        {
            return line.Substring(start, eos - start).Replace("\"", string.Empty);
        }

        static string[] getrec(string line, string delim)
        {
            // get record number
            int rn = 0;
            int lastrn = -1;
        ...
        public static bool setcol(List<string> headers, string colname, string line, string newval, out List<string> newrec) { return setcol(headers, colname, line, newval, out newrec, false); }
        public static bool setcol(List<string> headers, string colname, string line, string newval, out List<string> newrec, bool casesen)
        {
            newrec = new List<string>(headers.Count);
            int idx = getcolidx(headers, colname, casesen);
            if (idx < 0)
                return false;
            string[] r = getr(line);
            if (r.Length != headers.Count)
                return false;
        ...
        public static string[] getcsv(string fn, DebugDelegate d)
        {
            _deb = d;
            string file = getfile(fn, d);
            if ((file == null) || (file == string.Empty))
            {
                debug("no data found in: " + fn);
                return new string[0];
            }
            string[] lines = file.Split(Environment.NewLine.ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
            string[] nohead = new string[lines.Length - 1];
            Array.Copy(lines, 1, nohead, 0, lines.Length - 1);
            return nohead;

        }

        public static string getcol(List<string> headers, string colname, string line) { return getcol(headers, colname, line, false); }
        public static string getcol(List<string> headers, string colname, string line, bool casesen)
        {
            // get index
            int idx = getcolidx(headers, colname, casesen);
            if (idx < 0)
                throw new Exception("invalid column name: " + colname + " doesn't match: " + string.Join(",", headers));
            string[] r = getr(line);
            if (idx >= r.Length)
                throw new Exception("column name: " + colname + " with index: " + idx + " greater than records: " + r.Length + " in: " + line);
            return r[idx];
        }


        public static int getcolidx(List<string> headers, string colname)
        {
            return getcolidx(headers, colname, false);
        }
        public static int getcolidx(List<string> headers, string colname, bool casesense)
        {
            if (!casesense)
            {
                colname = colname.ToUpper();
                for (int i = 0; i < headers.Count; i++)
                    if (headers[i].ToUpper() == colname)
                        return i;
            }
            else
            {
        ...
        public static bool setfile(string fn, string data)
        {
            try
            {
